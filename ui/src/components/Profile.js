...
//To do: 
//1) Express (Image Rendering and Upload) --Done
//2) Social Media Icons --Done
//3) Cards for Interviews & Blogs --Done
//4) Dynamic User Call --Simple Change
//5) Reset Password Link 
//6) Icons for Skills --Done
...
return(
  <div className="bg-cyan-300 rounded overflow-hidden shadow-lg text-xl mb-2 w-1/2 mx-auto px-6 py-4">
  <div className="font-bold text-xl mb-2">User Profile</div>
  <div className="mb-5">
  <button className="bg-blue-500 hover:bg-blue-500 text-white font-bold border-black-500 py-2 px-2 rounded" onClick={handleCancel}>{isEdit ? "Cancel" : "Edit"}</button>
  </div>
  <form className="w-full max-w-lg user-profile-form">
  ...
  <img src={profilePhoto} className="w-3/5 lg:block hidden" alt="Profile" onClick={handleProfileChange}></img>
  ...
  <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label class="text-black-700 text-black-700 font-bold mb-2" for="first-name">First Name</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-white opacity-300" id="grid-first-name" type="text" name="firstName" value={currentProfileState?.firstName || ''} onChange={handleFieldChange} placeholder="Siddharth" disabled={!isEdit}></input>
    </div>
    <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label class="text-black-700 text-black-700 font-bold mb-2" for="last-name">Last Name</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-white opacity-300" id="grid-first-name" type="text" name="lastName" value={currentProfileState?.lastName || ''} onChange={handleFieldChange} placeholder="Prabhakaran" disabled={!isEdit}></input>
    </div>
    <div class="w-full rounded-lg sm:max-w-md xl:p-0">
      <label class="text-black-700 text-black-700 font-bold mb-2" for="phone-number">Phone Number</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-white opacity-300" id="phone" type="tel" name="phoneNumber"  value={currentProfileState?.phoneNumber || ''} onChange={handleFieldChange} placeholder="" disabled={!isEdit}></input>
    </div>
    <div class="w-full rounded-lg sm:max-w-md xl:p-0">
      <label class="text-black-700 text-black-700 font-bold mb-2" for="email-address">Email Address</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-gray opacity-300 cursor-not-allowed" id="email-address" type="email" name="email" value={currentProfileState?.email || ''} placeholder="example@gmail.com" disabled></input>
    </div>
    {/* TO DO: Make changes to the social media icons and skills to make it editable */}
    <div>
    {isEdit && <div class="w-full rounded-lg sm:max-w-md xl:p-0">
      <label class="text-black-700 text-black-700 font-bold mb-2" for="linkedin">Linkedin</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-white opacity-300" id="linkedin" type="text" name="linkedin" value={currentProfileState?.linkedin || ''} onChange={handleFieldChange} placeholder="Linkedin"></input>
    </div>}
    {!isEdit &&
      <button type="button" class="bg-blue-600 p-2 font-semibold text-white inline-flex items-center space-x-2 rounded" onClick={() => {
    ...
    <div>
    {isEdit &&  <div className="w-full rounded-lg sm:max-w-md xl:p-0">
    <label class="text-black-700 text-black-700 font-bold mb-2" for="github">GitHub</label>
        <input class="appearance-none block w-half bg-gray-200 text-black-700 border border-black-500 rounded py-3 px-10 mb-5 focus:outline-none focus:bg-white opacity-300" id="github" type="text" name="github" value={currentProfileState?.github || ''} onChange={handleFieldChange} placeholder="GitHub"></input>
    </div>}{!isEdit &&
      <button class="bg-gray-700 p-2 font-semibold text-white inline-flex items-center space-x-2 rounded"  onClick={() => {
        if(currentProfileState?.github)
        window.open(currentProfileState.github, {
          target: "_blank"
        })
      }}>
      <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" className="w-10 h-10 fill-current" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
    ...
    <div>
    {isEdit &&  <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label className="text-black-700 text-black-700 font-bold mb-2" for="twitter">Twitter</label>
    ...
    <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label className="text-black-700 text-black-700 font-bold mb-2" for="skills">Skills</label>
      {
    ...
    <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label className="text-black-700 text-black-700 font-bold mb-2" for="recent-interviews">Recent Interviews</label>
      {
    ...
    <div className="w-full rounded-lg sm:max-w-md xl:p-0">
      <label className="text-black-700 text-black-700 font-bold mb-2" for="blogs">Blogs</label>
      {
    ...
    <button type="submit" className="bg-blue-500 hover:bg-blue-500 text-white font-bold border-black-500 py-2 px-2 rounded" onClick={handleSubmit}>Save</button>
  </div>
  </form>
  </div>
  );
